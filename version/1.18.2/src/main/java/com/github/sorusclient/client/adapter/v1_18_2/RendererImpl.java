...
public class RendererImpl implements IRenderer {
    ...
    public void draw(RenderBuffer buffer) {
        var mode = switch (buffer.getDrawMode()) {
            case QUAD -> VertexFormat.DrawMode.QUADS;
        };
    ...
    public void drawRectangle(double x, double y, double width, double height, double cornerRadius, Color topLeftColor, Color bottomLeftColor, Color bottomRightColor, Color topRightColor) {
        var prevProgram = EventHook.INSTANCE.getLastBoundProgram();
        var prevBoundVertexArray = EventHook.getLastBoundArray();
        var prevBoundBuffer = EventHook.INSTANCE.getLastBoundBuffer();
        var prevBoundBufferTarget = EventHook.INSTANCE.getLastBoundBufferTarget();
    ...
    public void scissor(double x, double y, double width, double height) {
        ...
        RenderSystem.enableScissor(
                (int) (x * window.getWidth() / window.getScaledWidth()),
                (int) ((window.getScaledHeight() - (y + height)) * window.getHeight() / window.getScaledHeight()),
    ...
    static {
        var loadShaderIdentifier = IdentifierKt.toIdentifier("v1_18_2/net/minecraft/client/render/GameRenderer#loadShader(Lv1_18_2/net/minecraft/util/Identifier;)V");
        try {
            loadShader = GameRenderer.class.getDeclaredMethod(loadShaderIdentifier.getMethodName(), v1_18_2.net.minecraft.util.Identifier.class);
    ...
    public void loadBlur() {
        try {
            loadShader.invoke(MinecraftClient.getInstance().gameRenderer, new v1_18_2.net.minecraft.util.Identifier("sorus/shaders/blur.json"));
