...
public class HotBarTransformer implements Listener, ITransformer {

    private final String IN_GAME_HUD = Identifier.parse("v1_8_9/net/minecraft/client/gui/hud/InGameHud").getClassName();

    @Override
    public void run() {
        Sorus.getInstance().get(TransformerManager.class).register(HotBarTransformer.class);
    }
    ...
    private void transformInGameHud(ClassNode classNode) {
        ...
        for (MethodNode methodNode : classNode.methods) {
            if (methodNode.name.equals(renderHotBar.getMethodName()) && methodNode.desc.equals(renderHotBar.getMethodDesc())) {
